...
def fetch_reality_bazos(baseUrl, urlPath, urlQuery, adsPerPage):
    """
    Fetches all reality from bazos.cz
    return list of Reality objects
    """
    realities = []
    html = requests.get(f"{baseUrl}{urlPath}{urlQuery}")
    ...
    if pages > 1:
        for page in range(1, pages):
            html = requests.get(f"{baseUrl}{urlPath}/{page*adsPerPage}{urlQuery}")
            soup = BeautifulSoup(html.text, 'html.parser')
            main_content += soup.find_all(class_="inzeraty")

    # Process all fetched ads in threads
    pool = ThreadPoolExecutor(max_workers=10)
    futures = [pool.submit(_process_ad, ad, baseUrl) for ad in main_content]
...
def _process_ad(ad, base_url):
    ...
    if fetched_price in ['Dohodou','V textu','Nabídněte']:
        price = -1
    else:
        price = fetched_price.replace(' Kč', '').replace(' ', '')
    location = ad.find_all(class_="inzeratylok")[0].text
    url = base_url + ad.find_all(class_="inzeratynadpis")[0].find('a')['href']
